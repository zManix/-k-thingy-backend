...
export class TodoService extends BaseService {
  constructor(@InjectRepository(TodoEntity) private articleRepository: Repository<TodoEntity>) {
    super('todo.service');
  }

  async findAll(corrId: number): Promise<TodoEntity[]> {
  ...
  async findOne(corrId: number, id: number): Promise<TodoEntity> {
    const methodName = 'findOne';
    this.wl(corrId, methodName);
    const article: TodoEntity = await this.articleRepository.findOneBy({ id });
    ...
    this.wl(corrId, methodName, `we find todo item with id: ${id}. ret: ${JSON.stringify(article, null, 2)}`);
    return article;
  }

  async create(corrId: number, createTodoEntity: { title: string }): Promise<TodoEntity> {
    const method = 'create';
    this.wl(corrId, method);
    // extend the createDto with the timestamp
    const todo: { id: null; title: string } = { ...createTodoEntity, id: null };
    ...
    const identifier = (await this.articleRepository.insert(todo)).identifiers;
    const obj = await this.articleRepository.findOneBy(identifier[0]);
    this.wl(corrId, method, `create a new article item with id: ${todo.id}. ret: ${JSON.stringify(obj, null, 2)}`);
    return obj;
  }

  async replace(corrId: number, id: number, entity: { id: number; title: string }): Promise<TodoEntity> {
  ...
  async update(corrId: number, id: number, updateArticleEntity: UpdateTodoDto): Promise<TodoEntity> {
    const method = 'update';
    this.wl(corrId, method);
    const oldObj = await this.articleRepository.findOneBy({ id });
    // check if the id was found. find returns undefined in case nothing was found
  ...
  async remove(corrId: number, id: number): Promise<TodoEntity> {
  ...
  private checkForAllRequiredProperties(corrId: number, article: { id: number; title: string }) {
  ...
  private sanitizeTodoObject(corrId: number, article: { id: number; title: string }): TodoEntity {
